...
<ul class="breadcrumb">
    ...
    <li class="divider"></li>
    <li><a href="Member.php">Hội viên</a></li>
    <li class="divider"></li>
...
<div role="grid" class="dataTables_wrapper form-inline" id="DataTables_Table_0_wrapper">
    ...
    <div class="row-fluid1">
        <!--                form search-->
        <div>
            <form action="Association.php" method="post" accept-charset="utf-8">
                <div>
...
<div>
    <table
        ...
        if (isset($_REQUEST['page']) && (int)$_REQUEST['page']) {
            ...
            $records = $member->CountRowsAssociation();
            if ($records > $display) {
...
//                                $use2->page = $_REQUEST['page'];
        ...
        foreach ($info as $selectArrayItem) {
            ...
            <tr class="gradeA odd">
                ...
                <td class="center" style="vertical-align: middle"><?php echo $selectArrayItem->mem_association_number; ?></td>
            ...
            <ul>
                ...
                if ($page > 1) {
                    ...
                    if ($current != 1) {
                        ?>
                        <li>
                            <a href='Association.php?rows=<?php echo $display ?>&start=<?php echo $prev ?>'>Previous</a>
                        </li>
                    <?php
                    }
                    // Hien thi so link
                    for ($i = 1; $i <= $page; $i++) {
                        if ($current != $i) {
                            ?>
                            <li>
                                <a href='Association.php?rows=<?php echo $display ?>&start=<?php echo($display * ($i - 1)) ?>'><?php echo $i ?></a>
                            </li>
                        <?php
                        } else {
                            ?>
                            <li class="active"><a
                                    href='Association.php?rows=<?php echo $display ?>&start=<?php echo($display * ($i - 1)) ?>'><?php echo $i ?></a>
                            </li>
                        <?php
                        }
                    ...
                    if ($current != $page) {
                        ?>
                        <li>
                            <a href='Association.php?rows=<?php echo $display ?>&start=<?php echo $next ?>'>Next</a>
                        </li>
                    <?php
                    }
                }
...
<script>
    function Pagination() {
        location.href = "Association.php?rows=" + $('#pagination').val();
    }
</script>
